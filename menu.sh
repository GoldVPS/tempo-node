#!/bin/bash
# GoldVPS Tempo CLI
# - Jika RAM < 8GB -> otomatis buat 10GB swap (jika belum ada)
...
SWAPFILE="/swapfile"
SWAP_SIZE_GB=10        # create 10GB swap when RAM < 8GB
MIN_RAM_MB=8000        # threshold in MB to consider VPS "small"
DEFAULT_HTTP=8547
DEFAULT_WS=8548
DEFAULT_P2P=30304
...
function show_header() {
    ...
    echo " ╚═════╝  ╚═════╝ ╚══════╝╚═════╝   ╚═══╝  ╚═╝     ╚══════╝"
    echo -e "\e[0m"
    echo -e "🚀 \e[1;33mTempo Node Installer\e[0m - Powered by \e[1;33mGoldVPS Team\e[0m 🚀"
...
# === Swap creation for small VPS ===
function ensure_swap_for_small_vps() {
    ...
    # Only create swap when RAM < MIN_RAM_MB
...
# === Ensure ufw installed & open ports (no duplicates) ===
...
while true; do
    show_header
    echo -e "${BLUE_LINE}"
    echo -e "${CYAN}            Tempo Node Manager           ${RESET}"
    echo -e "${BLUE_LINE}"
    echo -e "  ${GREEN}1.${RESET} Install / Setup Tempo Node"
    echo -e "  ${GREEN}2.${RESET} Input Private Key & Wallet Address"
    echo -e "  ${GREEN}3.${RESET} Start Node"
    echo -e "  ${GREEN}4.${RESET} Check Status"
    echo -e "  ${GREEN}5.${RESET} View Logs"
    echo -e "  ${GREEN}6.${RESET} Test RPC"
    echo -e "  ${GREEN}7.${RESET} Restart Node"
    echo -e "  ${GREEN}8.${RESET} Stop Node"
    echo -e "  ${GREEN}9.${RESET} Update Node"
    echo -e "  ${GREEN}10.${RESET} Cleanup / Reset"
    echo -e "  ${GREEN}0.${RESET} Exit"
    echo -e "${BLUE_LINE}"
    read -p "Select an option (0–10): " option
...
TEMPO_HTTP_PORT=${DEFAULT_HTTP}
TEMPO_WS_PORT=${DEFAULT_WS}
TEMPO_P2P_PORT=${DEFAULT_P2P}
RUST_LOG=info
EOF
